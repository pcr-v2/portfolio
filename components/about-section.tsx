...
export function AboutSection() {
  return (
    <section
      id="about"
      className="min-h-screen flex flex-col justify-center py-20 px-6 md:px-12 lg:px-24 relative overflow-hidden"
    >
      <motion.div
      ...
      <div className="max-w-4xl w-full mx-auto">
        ...
        >
          ...
          >
            여러 프로젝트를 통해 ‘사용자가 겪는 순간’을 중심으로 개발해왔습니다. 기술은 화려하지 않아도, 쓰임새가 분명한
            선택을 하고자 합니다.
          </motion.p>
          ...
          >
            <motion.button
              whileHover={{ scale: 1.05, boxShadow: "0 20px 40px rgba(49, 150, 255, 0.3)" }}
              whileTap={{ scale: 0.95 }}
              onClick={() => document.getElementById("projects")?.scrollIntoView({ behavior: "smooth" })}
              className="px-8 py-4 bg-primary text-white rounded-full font-medium hover:shadow-lg hover:shadow-blue-500/25 transition-all flex items-center gap-2 group cursor-pointer"
            >
              View Projects
              <motion.div
                animate={{ x: [0, 5, 0] }}
            ...
            <motion.button
              ...
              className="px-8 py-4 bg-secondary text-foreground rounded-full font-medium hover:bg-secondary/80 transition-colors cursor-pointer"
            >
              Contact Me
            </motion.button>
          </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-20">
          ...
          >
            ...
            <div className="space-y-4 text-muted-foreground leading-relaxed">
              <p>
                저는 기능보다 경험을, 속도보다 방향을 먼저 고민하는 프론트엔드 개발자입니다. 프로젝트마다 다른 제약과
                환경을 경험하며, 기술 선택은 유행이 아니라 맥락과 문제 구조에 따라 달라지는 판단임을 배웠습니다.
              </p>
              <p>
                여러 B2C·B2B·외주 프로젝트를 거치며 사용자가 실제로 무엇을 느끼고, 어떤 흐름에서 불편함을 겪는지
                관찰하는 태도를 길렀습니다. 그래서 단순 구현보다 “왜 이 기능이 필요한가”를 먼저 정의하고, 가장 현실적인
                해법을 선택하는 개발을 지향합니다.
              </p>
              <p>
                협업에서는 소통과 명확성을 중심에 두며, 기획자·디자이너·운영자와 함께 문제를 정의하고 모두가 합의할 수
                있는 방향을 만드는 과정을 무엇보다 가치 있게 여깁니다.
              </p>
            </div>
          ...
          >
            ...
            <div className="space-y-6">
              {[
                ...
                { icon: Server, title: "Infra & Tools", desc: "AWS, Git, GitHub, GitHubAction, Jira, Slack, Notion" },
              ].map((skill, i) => (
                <motion.div
                  key={i}
                ...
                >
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h4 className="font-bold group-hover:text-primary transition-colors">{skill.title}</h4>
                  </div>
                  <p className="text-muted-foreground group-hover:text-foreground transition-colors">{skill.desc}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
